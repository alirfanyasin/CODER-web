...
@section('content')
    <section>
        <div class="breadcrumb d-flex justify-content-between align-items-center text-white">
            <h1>News</h1>
            {{-- <div>
        <a href="{{ route('admin.news.create') }}" class="btn-main">Create News</a>
      </div> --}}
            ...
            @foreach ($data as $news)
                <div class="col-md-4 mb-3">
                    <div class="p-3 text-white bg-custom">
                        <a href="{{ route('admin.news.show', $news->id) }}" class="text-decoration-none text-white">
                                ...
                                <ul class="dropdown-menu" style="z-index: 999;">
                                    <li>
                                        <a class="dropdown-item"
                                            href="{{ route('admin.news.edit', $news->id) }}"><iconify-icon
                                                icon="bx:edit"></iconify-icon>
                                            Edit</a>
                                    </li>
                                    <li>
                                        <form action="{{ route('admin.news.destroy', $news->id) }}" class="dropdown-item"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
